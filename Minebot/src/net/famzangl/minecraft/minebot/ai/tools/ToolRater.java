...
public class ToolRater {
	...
	private static class Depleted implements ItemFilter {
		@Override
		public boolean matches(ItemStack itemStack) {
			return itemStack != null && itemStack.isItemStackDamageable() && (itemStack.getMaxDamage() - itemStack.getItemDamage()) <= 1;
		}
	}
	...
	private static AndRater getCompoundAndRater(BlockFloatMap values, String string) {
	...
	private static Rater createRaterWithNot(String string,
	...
	private static Rater createRater(String name, BlockFloatMap values) {
		...
		throw new IllegalArgumentException("Could not find rater: "
				+ name);
	}
